...
import com.hzw.fdc.dao.MainFabWindowEndWindow2Dao
import com.hzw.fdc.function.PublicFunction.FdcKafkaSchema
import com.hzw.fdc.function.online.MainFabWindow2.{MainFabMatchControlPlanAndWindowsBroadcastProcessFunction, MainFabWindowEndCalcWindowBroadcastProcessFunction}
...
/**
 * MainFabLongRunWindowEndWindowService
 *
 * @desc:
 * @author tobytang
 * @date 2022/12/13 15:15
 * @since 1.0.0
 * @update 2022/12/13 15:15
 * */
class MainFabWindowEndWindow2Service extends TService{
  ...
  private val Dao = new MainFabWindowEndWindow2Dao
  ...
  override def analyses(): Any = {
    ...
    val controlPlanConfigDataStream = getDimDatas(ProjectConfig.KAFKA_MAINFAB_CONTROLPLAN_CONFIG_TOPIC,
      MainFabConstants.MAIN_FAB_WINDOWEND_WINDOW2_JOB_CONTROLPLAN_CONFIG_KAFKA_SOURCE_UID,
    ...
    val windowConfigDataStream = getDimDatas(ProjectConfig.KAFKA_MAINFAB_WINDOW2_CONFIG_TOPIC,
      MainFabConstants.MAIN_FAB_WINDOWEND_WINDOW2_JOB_WINDOW_CONFIG_KAFKA_SOURCE_UID,
      ProjectConfig.MAINFAB_PARTITION_DEFAULT)

    val indicatorConfigDataStream = getDimDatas(ProjectConfig.KAFKA_INDICATOR_CONFIG_TOPIC,
      MainFabConstants.MAIN_FAB_WINDOWEND_WINDOW2_JOB_INDICATOR_CONFIG_KAFKA_SOURCE_UID,
      ProjectConfig.MAINFAB_PARTITION_DEFAULT)

    val debugConfigStream = getDimDatas(ProjectConfig.KAFKA_DEBUG_CONFIG_TOPIC,
      MainFabConstants.MAIN_FAB_WINDOWEND_WINDOW2_JOB_DEBUG_CONFIG_KAFKA_SOURCE_UID)
    ...
    val runDataMatchedWindowStream = sourceDataStream.filter(inputValue => {
      ...
      .process(new MainFabMatchControlPlanAndWindowsBroadcastProcessFunction(MainFabConstants.windowTypeWindowEnd))
      .setParallelism(ProjectConfig.MAINFAB_WINDOWEND_WINDOW_MATCH_CONTROLPLAN_PARTITION)
      .name("WindowEnd  match CP and windows")
      .uid("WindowEnd  match CP and windows")
    ...
    matchControlPlanAndWindowGiveUpRunStream.addSink(new FlinkKafkaProducer(
      ...
      .name("WindowEnd matchCP giveUp sink")
      //添加uid用于监控
      .uid("WindowEnd matchCP giveUp sink")

    // todo 划window
    val windowEndWindowResultData = runDataMatchedWindowStream.keyBy(inputValue => {
      val traceId = inputValue.get(MainFabConstants.traceId).asText()
      ...
      .process(new MainFabWindowEndCalcWindowBroadcastProcessFunction())
      .setParallelism(ProjectConfig.MAINFAB_WINDOWEND_WINDOW_CALC_WINDOW_PARTITION)
      .name("WindowEnd calc window")
      .uid("WindowEnd calc window")

    // todo 收集cycleCount and dataMissingRatio Indicator 的数据
    val specialIndicator = windowEndWindowResultData.getSideOutput(specialIndicatorOutput)
    specialIndicator.addSink(new FlinkKafkaProducer(
      ...
      .name("WindowEnd specialIndicator sink")
      .uid("WindowEnd specialIndicator sink")

    // todo 划Window 时丢弃的数据 写入kafka
    val calcWindowGiveUpRunStream: DataStream[JsonNode] = windowEndWindowResultData.getSideOutput(calcWindowGiveUpRunStreamOutput)
    calcWindowGiveUpRunStream.addSink(new FlinkKafkaProducer(
      ...
      .name("WindowEnd calc window giveUp sink")
      //添加uid用于监控
      .uid("WindowEnd calc window giveUp sink")

    // todo 正常window结果数据 写入kafka
    windowEndWindowResultData.addSink(new FlinkKafkaProducer(
      ...
      .name("WindowEnd fdcWindowData sink")
      //添加uid用于监控
      .uid("WindowEnd fdcWindowData sink")


    // todo 日志信息 sink to kafka
    val mainFabLogInfo_calcWindow: DataStream[MainFabLogInfo] = windowEndWindowResultData.getSideOutput(mainFabLogInfoOutput)
    ...
    mainFabWindowLogInfos.addSink(new FlinkKafkaProducer(
      ...
      .name("WindowEnd MainFabLogInfo sink")
      .uid("WindowEnd MainFabLogInfo sink")


    // todo Debug 调试日志输出
    val mainFabDebugInfo_match = runDataMatchedWindowStream.getSideOutput(mainFabDebugInfoOutput)
    val mainFabDebugInfo_calcWindow = windowEndWindowResultData.getSideOutput(mainFabDebugInfoOutput)
    mainFabDebugInfo_match.union(mainFabDebugInfo_calcWindow).addSink(new FlinkKafkaProducer(
      ProjectConfig.KAFKA_MAINFAB_WINDOWEND_WINDOW_DEBUGINFO_TOPIC,
      new FdcKafkaSchema[MainFabDebugInfo](ProjectConfig.KAFKA_MAINFAB_WINDOWEND_WINDOW_DEBUGINFO_TOPIC
        //按照 traceId 分区
      ...
      .name("WindowEnd DebugInfo sink")
      //添加uid用于监控
      .uid("WindowEnd DebugInfo sink")
  ...
  override protected def getDatas(): DataStream[JsonNode] = {
    ...
    if(ProjectConfig.GET_KAFKA_DATA_BY_TIMESTAMP){
      getDao.getKafkaJsonSourcePartitionByTimestamp(
        ...
        ProjectConfig.KAFKA_CONSUMER_GROUP_WINDOWEND_WINDOW2_JOB,
        MainFabConstants.latest,
        ProjectConfig.MAINFAB_SOURCE_WINDOW_RAWDATA_PARTITION,
        MainFabConstants.MAIN_FAB_WINDOWEND_WINDOW2_JOB_RAWDATA_KAFKA_SOURCE_UID,
        MainFabConstants.MAIN_FAB_WINDOWEND_WINDOW2_JOB_RAWDATA_KAFKA_SOURCE_UID,
        ProjectConfig.KAFKA_MAINFAB_WINEDOWEND_WINDOW2_JOB_FROM_TIMESTAMP)
    }else{
