...
/**
 * @author ：gdj
 * @date ：Created in 2021/5/23 14:31
 * @description：${description }
 * @modified By：
 * @version: $version$
 */
object InitDataMissingIndicatorOracle {
  ...
  def initOracleConfig(logger: Logger): ListBuffer[ConfigData[IndicatorConfig]] = {
    ...
    try {
      ...
      while (resultSet.next) {
        if (resultSet.getString("ALGO_CLASS") == MainFabConstants.dataMissingRatio
          || resultSet.getString("ALGO_CLASS") == MainFabConstants.cycleCountIndicator) {
      ...
      resultSet.close()
      selectStmt.close()
      logger.warn("Oracle dataMissingRatio and cycleCountIndicator indicatorConfig SIZE :" + count)
      indicatorConfigDataByAll
    } catch {
      case e: Exception => logger.warn("dataMissingRatio and cycleCountIndicator indicatorConfig load error" + e)
        throw e
    } finally {
      if (conn != null) {
        conn.close()
