 ...
 */
class MainFabProcessEndConfigSplitKeyedBroadcastProcessFunction extends KeyedBroadcastProcessFunction[String,
  JsonNode, JsonNode, (String, JsonNode, JsonNode)] {
  private val logger: Logger = LoggerFactory.getLogger(classOf[MainFabProcessEndConfigSplitKeyedBroadcastProcessFunction])
  ...
  val job_fun_DebugCode:String = "023001"
  val jobName:String = "MainFabProcessEndWindowService"
  val optionName : String = "MainFabProcessEndConfigSplitKeyedBroadcastProcessFunction"
...
//            logger.warn("该traceId没有设置indicator: " + traceId)
            readOnlyContext.output(mainFabLogInfoOutput,
              generateMainFabLogInfo("0001A",
                "processElement",
                "该traceId没有设置indicator 1",
                Map[String,Any]("traceId" -> traceId,"dataInfo" -> in1)))
...
//          logger.warn("该traceId没有设置indicator: " + traceId)
          readOnlyContext.output(mainFabLogInfoOutput,
            generateMainFabLogInfo("0002A",
              "processElement",
              "该traceId没有设置indicator 2",
              Map[String,Any]("traceId" -> traceId,"dataInfo" -> in1)))
        }
      }

    }catch {
      case ex: Exception => logger.error(ErrorCode("0230010001C", System.currentTimeMillis(),
    ...
    try {
      ...
      if (dataType == MainFabConstants.context) {
        try {
          ...
          case e: Exception => logger.error(s"contextConfig json error data:$in2  Exception: ${ExceptionInfo.getExceptionInfo(e)}")
        }
        ...
        try {
          ...
          case e: Exception => logger.error(s"windowConfig json error  data:$in2  " +
            s"Exception: ${ExceptionInfo.getExceptionInfo(e)}")
        }
      }
    } catch {
      case e: Exception => logger.error(s"windowJob processBroadcastElement error data:$in2  " +
        s"Exception: ${ExceptionInfo.getExceptionInfo(e)}")
    }
  }
   ...
   */
  def addWindowConfigToWindowConfigMap(configList: ConfigData[List[WindowConfigData]]): Unit = {
    try {
      if(configList.datas.isEmpty){
        return
      }
      //logger.warn(s"addWindowConfigToWindowConfigMap: $configList")
      ...
      for (elem <- configList.datas) {
        if(elem.calcTrigger == "ProcessEnd") {
          ...
          try {
            ...
            case ex: Exception => logger.error(ErrorCode("0230010002C", System.currentTimeMillis(),
      ...
      totalKeySet.foreach(key => {
        if(elemSensorConfig.contains(key)) {
          ...
          //splitSensorFunctionByHash(chamerSensorSet.to[mutable.Set], key)
      ...
      case ex: Exception => logger.error(ErrorCode("0230010003C", System.currentTimeMillis(),
  ...
  def splitSensorFunction(chamerSensorSet: mutable.Set[String], key: String): Unit = {
    try {
      ...
      for (elem <- chamerSensorSet) {
        // 获取sensorPartitionID
        val sensorPartitionID = (elem.hashCode % ProjectConfig.PROCESSEND_SENSOR_SPLIT_GROUP_COUNT).abs.toLong
        if (sensorPartitionList.contains(key)) {
      ...
      case ex: Exception => logger.error(ErrorCode("0230010005C", System.currentTimeMillis(),
      ...
      try {
        ...
        for (elem <- ElemWindowConfigAliasMap) {
          try {
            ...
            case ex: Exception => logger.error(s"WindowConfigData error: ${key} | ${elem._1} | ${ex.toString}")
          }
        }
        ...
        def parseWindowConfig(resWindowConfigDataNew: WindowConfigData): Unit = {
          try {
            ...
            case ex: Exception => logger.error(ErrorCode("0230010006C", System.currentTimeMillis(),
        ...
        case ex: Exception => logger.error(ErrorCode("0230010007C", System.currentTimeMillis(),
  ...
  def getExtraSensorFunction(key: String): Unit = {
    try{
      ...
      case ex: Exception => logger.error(ErrorCode("0230010008C", System.currentTimeMillis(),
  ...
  def modifyElemSensorConfig(keySet: mutable.Set[String], windowConfigData: WindowConfigData, status: Boolean): Unit = {
    try{
      ...
      case ex: Exception => logger.error(ErrorCode("0230010009C", System.currentTimeMillis(),
  ...
  def modifyWindowConfigMap(keySet: mutable.Set[String], windowConfigData: WindowConfigData, status: Boolean): Unit = {
    try{
      ...
      case ex: Exception => logger.error(ErrorCode("0230010010C", System.currentTimeMillis(),
  ...
  def addContextMapToContextMap(configList: ConfigData[List[ContextConfigData]]): Unit = {
    try {
      ...
      case e: Exception =>
        logger.error(ErrorCode("0230010011C", System.currentTimeMillis(), Map(), e.toString).toJson)
    }
  }
